...
def repeat_all_sort(folderpath: str = os.getcwd(), threshold: int = 0, hardlinks: bool = True) -> None:
    '''Sorts the images in the folderpath by their keywords. Image is sorted into every keyword.'''
...
def most_popular_sort(folderpath: str = os.getcwd(), threshold: int = 0, hardlinks: bool = True) -> None:
    '''Sorts the images in the folderpath by their most popular keywords. Only sorts the image into most popular keyword.'''
    keys = keyword_frequency(folderpath)
    for file in os.listdir(folderpath):
        if os.path.isfile(file) and ft.image_match(file) != None:
            img = ExifEditor(file)
